...
async function loadData() {
    if(isFirstLoad) 
    {
        showLoading(); // Hiển thị overlay loading khi bắt đầu tải dữ liệu
    }
...
for (let i = 2; i < 52; i++) {
    ...
    if (row1) {
        ...
        if(isFirstLoad) {
        hideLoading(); // Ẩn overlay loading sau khi hoàn thành (dù thành công hay lỗi)
        isFirstLoad = false; // Đánh dấu đã hoàn thành lần tải đầu tiên
    }
    }
}
function startAutoLoad() {
    if (autoLoad) clearInterval(autoLoad);
    loadData(); // Gọi ngay để không phải đợi 5 giây đầu tiên
...
// Hàm hiển thị và ẩn overlay loading
let isFirstLoad = true; // Biến để kiểm tra lần tải đầu tiên
function showLoading() {
    // Dùng 'flex' để căn giữa nội dung bên trong thay vì 'block'
