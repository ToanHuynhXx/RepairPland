const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxQYSWqX3SyNM7pdR3d64GBUMjf_f_zyDvbLcEVs7tFerW4ysZvsl_VEoyQUoJwU0k3/exec";
    ...
    function applyColoring() {
        ...
        rows.forEach(function(row) {
            ...
            if (cells.length >= 13) {
                ...
                let statusValue = cells[12].textContent.trim();
...
async function loadData() {
    try {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            row.insertCell(8).innerText = formatDateDisplay(data[i][9]); // Ngay mua hang
            row.insertCell(9).innerText = formatDateDisplay(data[i][10]); // Ngay kaizen sửa
            row.insertCell(10).innerText = formatDateDisplay(data[i][11]); // Ngay ncc sưa
            row.insertCell(11).innerText = data[i][12]*1 + "%"; // Ngay hoan thanh
            row.insertCell(12).innerText = data[i][14]; // Trang thai           
...
async function updateRow(button, oldStatus) {
    ...
    let TGKiemTra = "";
    let TGSuaChua = "";
    if(row.cells[12].querySelector(".edit-status").value === "hoan thanh")
    {
        if (!Linhkien) {
