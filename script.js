const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxLOsYGAGUldoIPY75hM3xdCmX9U7E-dXoTWGZ9e4vt59FTY1psgajVaKA5kvfGv5FO/exec";
...
async function sendSingleRow(button) {
    ...
    const DateKHKZ = row.querySelector(".row-date2").value;
    const DateTN = row.querySelector(".row-date3").value;
    const DateMua = row.querySelector(".row-date4").value;
    const DateKZ = row.querySelector(".row-date5").value;
    const DateNCC = row.querySelector(".row-date6").value;
...
async function updateRow(button, oldStatus) {
    ...
    let TGKiemTra = "";
    let TGSuaChua = "";
    if(row.cells[13].querySelector(".edit-status").value === "hoan thanh")
    {
        if (!Linhkien) {
            alert("Vui lòng nhập linh kiện thay thế!");
            return;
        }
        TGSuaChua = prompt("Nhập thời gian đã sửa chữa (số ngày):");
        if (TGSuaChua === null || TGSuaChua === "") {
            alert("Vui lòng nhập thời gian đã sửa chữa!");
            return;
        }
    }
    if (row.cells[13].querySelector(".edit-status").value === "Kiểm tra" && oldStatus !== "Kiểm tra") {
        tiepNhan = new Date().toISOString().split('T')[0]; // Lấy ngày hiện tại theo định dạng yyyy-mm-dd
    }
    else tiepNhan = formatDate(row.cells[8].querySelector(".edit-date3").value); // Nếu không phải "Kiểm tra" thì giữ nguyên ngày cũ
    if (row.cells[13].querySelector(".edit-status").value === "Chờ linh kiện"&& oldStatus !== "Chờ linh kiện")
    {
        TGKiemTra = prompt("Nhập thời gian đã kiểm tra (số ngày):");
        if (TGKiemTra === null || TGKiemTra === "") {
            alert("Vui lòng nhập thời gian đã kiểm tra!");
            return;
        }
        muaHang = new Date().toISOString().split('T')[0]; // Lấy ngày hiện tại theo định dạng yyyy-mm-dd
    }
    else muaHang = formatDate(row.cells[9].querySelector(".edit-date4").value); // Nếu không phải "Chờ linh kiện" thì giữ nguyên ngày cũ
    if (row.cells[13].querySelector(".edit-status").value === "Kaizen sửa"&& oldStatus !== "Kaizen sửa") {
        kaizen = new Date().toISOString().split('T')[0]; // Lấy ngày hiện tại theo định dạng yyyy-mm-dd
    }
    else kaizen = formatDate(row.cells[10].querySelector(".edit-date5").value); // Nếu không phải "Kaizen sửa" thì giữ nguyên ngày cũ
    if (row.cells[13].querySelector(".edit-status").value === "NCC sửa"&& oldStatus !== "NCC sửa") {
        nCC = new Date().toISOString().split('T')[0]; // Lấy ngày hiện tại theo định dạng yyyy-mm-dd
    }
    else nCC = formatDate(row.cells[11].querySelector(".edit-date6").value); // Nếu không phải "NCC sửa" thì giữ nguyên ngày cũ
    const updatedData = {
        action: "update",
        rowIndex: rowIndex,
    ...
    fetch(SCRIPT_URL)
        ...
        .then(responseObj => {
            ...
            if(responseObj && responseObj.dataSelect){
                document.getElementById('btn-1').innerText = responseObj.dataSelect[0];
                document.getElementById('btn-2').innerText = responseObj.dataSelect[1];
                document.getElementById('btn-3').innerText = responseObj.dataSelect[2];
                document.getElementById('btn-4').innerText = responseObj.dataSelect[3];
                document.getElementById('btn-5').innerText = responseObj.dataSelect[4];
                document.getElementById('btn-6').innerText = responseObj.dataSelect[5];
                document.getElementById('btn-7').innerText = responseObj.dataSelect[6];
                document.getElementById('btn-8').innerText = responseObj.dataSelect[7];
                document.getElementById('btn-9').innerText = responseObj.dataSelect[8];
            }
            })
        .catch(error =>{ console.error('Lỗi:', error);
             document.getElementById('btn-1').innerText = "ErrorLoadData";
             document.getElementById('btn-2').innerText = "ErrorLoadData";
             document.getElementById('btn-3').innerText = "ErrorLoadData";
             document.getElementById('btn-4').innerText = "ErrorLoadData";
             document.getElementById('btn-5').innerText = "ErrorLoadData";
             document.getElementById('btn-6').innerText = "ErrorLoadData";
             document.getElementById('btn-7').innerText = "ErrorLoadData";
             document.getElementById('btn-8').innerText = "ErrorLoadData";
             document.getElementById('btn-9').innerText = "ErrorLoadData";
        });
}
